...
import argparse, json, os, folium, logging
...
class DataHandler:
    ...
    def _validate(self, df):
        ...
        opt = ["package_size", "delivery_priority", "time_window_start", "time_window_end"]
        ...
        for col in [c for c in opt if c not in df.columns]:
            today = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
            if col == "package_size": df[col] = 1
            elif col == "delivery_priority": df[col] = 3
            elif col == "time_window_start": df[col] = today + timedelta(hours=8)
            elif col == "time_window_end": df[col] = today + timedelta(hours=18)
...
class StirlingOptimizer:
    ...
    def find_optimal_k(n, a, b, min_k=1, max_k=None):
        ...
        costs = [(k, StirlingOptimizer.generalized_stirling(n, k, a, b)) for k in range(min_k, min(n+1, max_k+1))]
...
class RouteSolver:
    ...
    def constructive_approach(self, k):
        ...
        return {i: self.data[self.data['route_constructive'] == i].index.tolist() for i in range(k)}
    def deconstructive_approach(self, k):
        G = nx.Graph(); n = len(self.data)
        for i in range(n): G.add_node(i, pos=(self.data.iloc[i]['latitude'], self.data.iloc[i]['longitude']))
        threshold = np.percentile(self.distance_matrix.flatten(), 15)
        for i in range(n):
            for j in range(i+1, n):
                if self.distance_matrix[i, j] < threshold: G.add_edge(i, j, weight=self.distance_matrix[i, j])
        ...
        return {i: self.data[self.data['route_deconstructive'] == i].index.tolist() for i in range(k)}, G
    def optimize_route(self, points, method="nearest_neighbor"):
        if len(points) <= 2: return points
        sub_matrix = self.distance_matrix[np.ix_(points, points)]
        if method == "nearest_neighbor":
            ordered = [0]; unvisited = set(range(1, len(points)))
            while unvisited:
                current = ordered[-1]
                next_idx = min(unvisited, key=lambda x: sub_matrix[current, x])
                ordered.append(next_idx); unvisited.remove(next_idx)
...
class ReportGenerator:
    ...
    def _create_cost_plot(self):
        ...
        if "all_costs" in c:
            plt.plot([x[0] for x in c["all_costs"]], [x[1] for x in c["all_costs"]], 'b-o', label=f"Constructive (a={c['parameters']['a']:.2f}, b={c['parameters']['b']:.2f})")
            plt.axvline(x=c["optimal_k"], color='b', linestyle='--')
        if "all_costs" in d:
            plt.plot([x[0] for x in d["all_costs"]], [x[1] for x in d["all_costs"]], 'r-o', label=f"Deconstructive (a={d['parameters']['a']:.2f}, b={d['parameters']['b']:.2f})")
    ...
    def _create_route_map(self, graph=None):
        ...
        for approach, layer_name in [("constructive", "Constructive"), ("deconstructive", "Deconstructive")]:
            ...
            for route_id in range(self.results[approach]["optimal_k"]):
                ...
                if len(ordered_points) > 1:
                    ...
                    folium.PolyLine(locations=route_coords, color=color, weight=3, opacity=0.7, dash_array='5,10' if approach == "deconstructive" else None).add_to(route_group)
                for _, point in route_points.iterrows():
                    folium.CircleMarker(location=[point['latitude'], point['longitude']], radius=5, color=color, fill=True, fill_color=color, popup=f"ID: {point['id']}").add_to(route_group)
        ...
        if graph is not None:
            ...
            for u, v, _ in graph.edges(data=True):
                ...
                folium.PolyLine(locations=[(u_lat, u_lon), (v_lat, v_lon)], color='gray', weight=1, opacity=0.5).add_to(graph_layer)
    ...
    def _create_comparison_report(self):
        ...
        c_metrics = {"min_pts": min(c_pts), "max_pts": max(c_pts), "avg_pts": np.mean(c_pts), "std_pts": np.std(c_pts), "avg_dist": c["total_distance"] / c["optimal_k"], "pts_per_dist": len(self.data) / c["total_distance"]}
        d_metrics = {"min_pts": min(d_pts), "max_pts": max(d_pts), "avg_pts": np.mean(d_pts), "std_pts": np.std(d_pts), "avg_dist": d["total_distance"] / d["optimal_k"], "pts_per_dist": len(self.data) / d["total_distance"]}
        ...
        <table>
            ...
            <tr><td>Routes</td><td>{c["optimal_k"]}</td><td>{d["optimal_k"]}</td>
                <td class="winner">{"Deconstructive" if d["optimal_k"] < c["optimal_k"] else "Constructive" if c["optimal_k"] < d["optimal_k"] else "Tie"}</td></tr>
            <tr><td>Total Distance</td><td>{c["total_distance"]:.2f}</td><td>{d["total_distance"]:.2f}</td>
                <td class="winner">{"Deconstructive" if d["total_distance"] < c["total_distance"] else "Constructive" if c["total_distance"] < d["total_distance"] else "Tie"}</td></tr>
            <tr><td>Balance (std)</td><td>{c_metrics["std_pts"]:.2f}</td><td>{d_metrics["std_pts"]:.2f}</td>
                <td class="winner">{"Deconstructive" if d_metrics["std_pts"] < c_metrics["std_pts"] else "Constructive" if c_metrics["std_pts"] < d_metrics["std_pts"] else "Tie"}</td></tr>
        ...
        with open("visualizations/delivery_report.html", "w") as f: f.write(html)
...
class DualPerspectiveOptimizer:
    ...
    def constructive_approach(self, min_vehicles=1, max_vehicles=None):
        ...
        for i, indices in routes_dict.items():
            ...
            total_distance = sum(self.route_solver.distance_matrix[ordered_indices[j], ordered_indices[j+1]] for j in range(len(ordered_indices)-1)) if len(ordered_indices) > 1 else 0
            routes[i] = {"point_count": len(route_points), "total_distance": total_distance, "points": route_points['id'].tolist(), "ordered_indices": ordered_indices}
        self.results["constructive"] = {"parameters": {"a": a, "b": b}, "optimal_k": k_opt, "optimal_cost": cost_opt, "all_costs": all_costs, "routes": routes, "total_distance": sum(r["total_distance"] for r in routes.values())}
    ...
    def deconstructive_approach(self, min_vehicles=1, max_vehicles=None):
        ...
        for i, indices in routes_dict.items():
            ...
            total_distance = sum(self.route_solver.distance_matrix[ordered_indices[j], ordered_indices[j+1]] for j in range(len(ordered_indices)-1)) if len(ordered_indices) > 1 else 0
            routes[i] = {"point_count": len(route_points), "total_distance": total_distance, "points": route_points['id'].tolist(), "ordered_indices": ordered_indices}
        self.results["deconstructive"] = {"parameters": {"a": a, "b": b}, "optimal_k": k_opt, "optimal_cost": cost_opt, "all_costs": all_costs, "routes": routes, "total_distance": sum(r["total_distance"] for r in routes.values()), "graph": G}
    ...
    def save_results(self):
        def to_serializable(obj):
            ...
            if isinstance(obj, tuple) and len(obj) == 2: return [to_serializable(obj[0]), to_serializable(obj[1])]
            return obj
        results_copy = {"constructive": self.results["constructive"].copy(), "deconstructive": self.results["deconstructive"].copy()}
        if "graph" in results_copy["deconstructive"]: del results_copy["deconstructive"]["graph"]
        os.makedirs("results", exist_ok=True)
        with open("results/optimization_results.json", "w") as f: json.dump(to_serializable(results_copy), f, indent=2)
...
def generate_sample_data(n=50, seed=42):
    ...
    df = pd.DataFrame({'id': range(1, n+1), 'latitude': lats, 'longitude': longs, 'package_size': package_sizes, 'delivery_priority': priorities, 'time_window_start': time_windows_start, 'time_window_end': time_windows_end})
...
if __name__ == "__main__":
    ...
    if data_path:
        ...
        Args:
            points: List of point indices to optimize
            method: Method to use for optimization ("tsp", "nearest_neighbor", etc.)
            
        Returns:
            Ordered list of point indices
        """
        if len(points) <= 2:
            return points
            
        # Extract the distance submatrix for these points
        sub_matrix = self.distance_matrix[np.ix_(points, points)]
        ...
        # For more complex instances, could use OR-Tools or other TSP solvers
        elif method == "tsp":
            try:
                # Try to use NetworkX's approximation
                G = nx.Graph()
                for i in range(len(points)):
                    for j in range(i+1, len(points)):
                        G.add_edge(i, j, weight=sub_matrix[i, j])
                
                # Approximate TSP solution
                tsp_path = nx.approximation.traveling_salesman_problem(G, cycle=True)
                
                # Remove the last point to avoid returning to start
                if len(tsp_path) > 1 and tsp_path[0] == tsp_path[-1]:
                    tsp_path = tsp_path[:-1]
                
                # Map back to original indices
                return [points[i] for i in tsp_path]
            except Exception as e:
                logger.warning(f"TSP optimization failed: {e}. Falling back to nearest neighbor.")
                return self.optimize_route(points, method="nearest_neighbor")
        
        # Default: return points in original order
        return points


class ReportGenerator:
    """Handles visualization and reporting of route optimization results."""
    ...
    def visualize_routes(self, graph: Optional[nx.Graph] = None) -> None:
        """
        Create visualizations for both approaches.
        
        Args:
            graph: NetworkX graph for the deconstructive approach (optional)
        """
        if self.data is None:
            logger.warning("No data available for visualization")
            return
            
        # Plot cost vs k for both approaches
        self._create_cost_plot()
        
        # Create map visualization
        self._create_route_map(graph)
        
        # Create comparison table as HTML
        self._create_comparison_report()
    
    def _create_cost_plot(self) -> None:
        """Create a plot comparing costs vs. k for both approaches."""
        plt.figure(figsize=(12, 6))
        
        if "all_costs" in self.results["constructive"]:
            c_costs = self.results["constructive"]["all_costs"]
            plt.plot([x[0] for x in c_costs], [x[1] for x in c_costs], 'b-o', 
                    label=f"Constructive (a={self.results['constructive']['parameters']['a']:.2f}, b={self.results['constructive']['parameters']['b']:.2f})")
            plt.axvline(x=self.results["constructive"]["optimal_k"], color='b', linestyle='--', 
                       label=f"Optimal k = {self.results['constructive']['optimal_k']}")
        
        if "all_costs" in self.results["deconstructive"]:
            d_costs = self.results["deconstructive"]["all_costs"]
            plt.plot([x[0] for x in d_costs], [x[1] for x in d_costs], 'r-o', 
                    label=f"Deconstructive (a={self.results['deconstructive']['parameters']['a']:.2f}, b={self.results['deconstructive']['parameters']['b']:.2f})")
            plt.axvline(x=self.results["deconstructive"]["optimal_k"], color='r', linestyle='--', 
                       label=f"Optimal k = {self.results['deconstructive']['optimal_k']}")
        
        plt.xlabel("Number of Routes (k)")
        plt.ylabel("Total Cost")
        plt.title("Generalized Stirling Optimization: Cost vs. Number of Routes")
        plt.legend()
        plt.grid(True)
        plt.savefig("visualizations/cost_comparison.png", dpi=300, bbox_inches='tight')
    
    def _create_route_map(self, graph: Optional[nx.Graph] = None) -> None:
        """
        ...
        # Constructive routes
        if 'route_constructive' in self.data.columns:
            for route_id in range(self.results["constructive"]["optimal_k"]):
                route_points = self.data[self.data['route_constructive'] == route_id]
                color = colors[route_id % len(colors)]
                
                # Create a route group
                route_group = folium.FeatureGroup(name=f"Constructive Route {route_id+1}")
                
                # Add polyline for the route if we have an ordered route
                ordered_points = route_points.sort_values('id')  # Default to ID order
                if len(ordered_points) > 1:
                    ...
                    folium.PolyLine(
                        ...
                        weight=3,
                        opacity=0.7,
                        popup=f"Route {route_id+1} - {len(route_points)} points"
                    ).add_to(route_group)
                ...
                for _, point in route_points.iterrows():
                    folium.CircleMarker(
                        ...
                        fill=True,
                        fill_color=color,
                        popup=f"ID: {point['id']}, Route: {route_id+1} (Constructive)"
                    ).add_to(route_group)
                
                route_group.add_to(m)
        
        # Deconstructive routes
        if 'route_deconstructive' in self.data.columns:
            for route_id in range(self.results["deconstructive"]["optimal_k"]):
                route_points = self.data[self.data['route_deconstructive'] == route_id]
                color = colors[route_id % len(colors)]
                ...
                if len(ordered_points) > 1:
                    ...
                    folium.PolyLine(
                        ...
                        weight=3,
                        opacity=0.7,
                        dash_array='5,10',  # Dashed line for deconstructive
                        popup=f"Route {route_id+1} - {len(route_points)} points"
                    ).add_to(route_group)
                ...
                for _, point in route_points.iterrows():
                    folium.CircleMarker(
                        ...
                        fill_opacity=0.5,
                        fill_color=color,
                        popup=f"ID: {point['id']}, Route: {route_id+1} (Deconstructive)",
                        weight=2,
                        dash_array='5,5'  # Dashed line to distinguish
                    ).add_to(route_group)
                
                route_group.add_to(m)
                
        # If we have the graph for the deconstructive approach, visualize it
        if graph is not None:
            graph_layer = folium.FeatureGroup(name="Network Graph", show=False)
            
            # Add edges as polylines
            for u, v, data in graph.edges(data=True):
                if u >= len(self.data) or v >= len(self.data):
    ...
    def _create_comparison_report(self) -> None:
        """Create an HTML report comparing the approaches."""
        if not self.results["constructive"] or not self.results["deconstructive"]:
            logger.warning("Missing results for comparison report")
            return
        ...
        c_points_per_route = [r["point_count"] for r in c["routes"].values()]
        d_points_per_route = [r["point_count"] for r in d["routes"].values()]
        
        c_metrics = {
            "min_points": min(c_points_per_route),
            "max_points": max(c_points_per_route),
            "avg_points": np.mean(c_points_per_route),
            "std_points": np.std(c_points_per_route),
            "avg_distance": c["total_distance"] / c["optimal_k"],
            "points_per_distance": len(self.data) / c["total_distance"]
        }
        
        d_metrics = {
            "min_points": min(d_points_per_route),
            "max_points": max(d_points_per_route),
            "avg_points": np.mean(d_points_per_route),
            "std_points": np.std(d_points_per_route),
            "avg_distance": d["total_distance"] / d["optimal_k"],
            "points_per_distance": len(self.data) / d["total_distance"]
        }
        ...
        <html>
        <head>
            <title>Delivery Route Optimization Report</title>
            <style>
                ...
                .chart {{ margin: 20px; max-width: 600px; }}
                .metrics {{ display: flex; flex-wrap: wrap; }}
                .metric-card {{ background-color: #f8f9fa; border-radius: 5px; 
                               padding: 15px; margin: 10px; flex: 1; min-width: 200px; }}
                .metric-value {{ font-size: 24px; font-weight: bold; color: #333366; }}
                .improvement {{ color: green; }}
                .explanation {{ background-color: #f0f7ff; padding: 15px; border-left: 4px solid #3498db; margin: 20px 0; }}
            </style>
        </head>
        <body>
            <h1>Delivery Route Optimization Report</h1>
            ...
            <div class="metrics">
                <div class="metric-card">
                    <h3>Total Delivery Points</h3>
                    <div class="metric-value">{len(self.data)}</div>
                </div>
                <div class="metric-card">
                    <h3>Vehicles Needed</h3>
                    <div class="metric-value">
                        {min(c["optimal_k"], d["optimal_k"])}
                        <span class="improvement">
                            (-{abs(c["optimal_k"] - d["optimal_k"])})
                        </span>
                    </div>
                </div>
                <div class="metric-card">
                ...
                <div class="metric-card">
                    <h3>Points Per Route</h3>
                    <div class="metric-value">
                        {min(c_metrics["avg_points"], d_metrics["avg_points"]):.1f}
                    </div>
                </div>
            </div>
            
            <div class="explanation">
                <h3>Understanding Parameters (a,b)</h3>
                <p><strong>Affinity Parameter (a)</strong>: Controls how elements are connected.</p>
                <ul>
                    <li>In the <strong>constructive approach</strong>, it represents the cost of connecting delivery points.</li>
                    <li>In the <strong>deconstructive approach</strong>, it represents the cost of separating connected points.</li>
                </ul>
                <p><strong>Barrier Parameter (b)</strong>: Controls boundaries between groups.</p>
                <ul>
                    <li>In the <strong>constructive approach</strong>, it represents the cost of establishing a new route.</li>
                    <li>In the <strong>deconstructive approach</strong>, it represents the cost of creating a boundary between segments.</li>
                </ul>
            </div>
            ...
            <table>
                <tr>
                    <th>Metric</th>
                    <th>Constructive Approach</th>
                    <th>Deconstructive Approach</th>
                ...
                <tr>
                    ...
                    <td>{d["optimal_k"]}</td>
                    <td class="winner">{
                        "Deconstructive" if d["optimal_k"] < c["optimal_k"] else 
                        "Constructive" if c["optimal_k"] < d["optimal_k"] else "Tie"
                    }</td>
                </tr>
                <tr>
                    <td>Total Distance</td>
                    <td>{c["total_distance"]:.2f}</td>
                    <td>{d["total_distance"]:.2f}</td>
                    <td class="winner">{
                ...
                <tr>
                    <td>Route Balance (std dev)</td>
                    <td>{c_metrics["std_points"]:.2f}</td>
                    <td>{d_metrics["std_points"]:.2f}</td>
                    <td class="winner">{
                        "Deconstructive" if d_metrics["std_points"] < c_metrics["std_points"] else 
                        "Constructive" if c_metrics["std_points"] < d_metrics["std_points"] else "Tie"
                    }</td>
                </tr>
                <tr>
                    <td>Avg. Points per Route</td>
                    <td>{c_metrics["avg_points"]:.2f}</td>
                    <td>{d_metrics["avg_points"]:.2f}</td>
                    <td>—</td>
                </tr>
                <tr>
                    <td>Min/Max Points per Route</td>
                    <td>{c_metrics["min_points"]}/{c_metrics["max_points"]}</td>
                    <td>{d_metrics["min_points"]}/{d_metrics["max_points"]}</td>
                    <td class="winner">{
                        "Deconstructive" if d_metrics["max_points"] - d_metrics["min_points"] < c_metrics["max_points"] - c_metrics["min_points"] else 
                        "Constructive" if c_metrics["max_points"] - c_metrics["min_points"] < d_metrics["max_points"] - d_metrics["min_points"] else "Tie"
                    }</td>
                </tr>
                <tr>
                    <td>Avg. Distance per Route</td>
                    <td>{c_metrics["avg_distance"]:.2f}</td>
                    <td>{d_metrics["avg_distance"]:.2f}</td>
                    <td class="winner">{
                        "Deconstructive" if d_metrics["avg_distance"] < c_metrics["avg_distance"] else 
                        "Constructive" if c_metrics["avg_distance"] < d_metrics["avg_distance"] else "Tie"
                    }</td>
                </tr>
                <tr>
                    <td>Efficiency (Points/Distance)</td>
                    <td>{c_metrics["points_per_distance"]:.2f}</td>
                    <td>{d_metrics["points_per_distance"]:.2f}</td>
                    <td class="winner">{
                        "Deconstructive" if d_metrics["points_per_distance"] > c_metrics["points_per_distance"] else 
                        "Constructive" if c_metrics["points_per_distance"] > d_metrics["points_per_distance"] else "Tie"
                    }</td>
                </tr>
                <tr>
                    <td>Parameters (a, b)</td>
                    <td>({c["parameters"]["a"]:.2f}, {c["parameters"]["b"]:.2f})</td>
                    <td>({d["parameters"]["a"]:.2f}, {d["parameters"]["b"]:.2f})</td>
